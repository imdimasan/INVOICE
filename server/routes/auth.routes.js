...
router.post(
  "/register",
  [
    check("email", "Непраивльный email").isEmail(),
    check("password", "Минимальная длина минимум 6 символов").isLength({
      min: 6,
    }),
  ],
  async (req, res) => {
    try {
      ...
      if (candidate) {
        return res
          .status(400)
          .json({ usererr: "Такой пользователь уже создан" });
      }
...
router.post(
  "/login",
  [
    check("email", "Введте корректный email").normalizeEmail().isEmail(),
