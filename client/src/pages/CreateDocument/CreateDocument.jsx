...
function CreateDocument () {
    ...
    <div className="wrapper">
        ...
        <Buttons className="main__buttons_btn" onClick={()=> {setDocType("Акт выполненных работ")}}>Акт выполненных работ</Buttons>
...
></Input>
        ...
        <div className="main__document" id="element-to-print">
          <div className="doc__header">
            ...
            <div className="doc__header_info">
              <p>
                {userInfo.unp}<br></br>
                {userInfo.organization}<br></br>
                {userInfo.legaladdress}<br></br>
              </p>
            </div>
          </div>
          <div className="doc__row">
            <div className="doc__type col-1"><p>{docType}</p></div>
            <div className="doc__number_p col-2"><p>№</p></div>
            <div className="doc__number col-3"><p>{new Intl.DateTimeFormat('ru-RU').format(new Date(docDate.date)).replace(/[.]/g,"")}</p></div>
            <div className="doc__date_p col-4"><p>от</p></div>
            <div className="doc__date col-5"><p>{new Intl.DateTimeFormat('ru-RU').format(new Date(docDate.date))}</p></div>
          </div>
          <div className="doc__row">
            <div className="contract col-1"><p>Основание: договор</p></div>
            <div className="contract__number_p col-2"><p>№</p></div>
            <div className="contract__number col-3"><p>{clientinfo.contractname}</p></div>
            <div className="contract__date_p col-4"><p>от</p></div>
            <div className="contract__date col-5"><p>{clientinfo.contractdate? new Intl.DateTimeFormat('ru-RU').format(new Date(clientinfo.contractdate)) : ""}</p></div>
          </div>
          ...
          <div className="doc__body">
            ...
            <div className="col-2">
              <p><strong>Наименование работы (услуги)</strong></p>
            </div>
            {/* <div className="col-3">
          ...
          />
            ...
            <div className="doc__body__qty col-4"><p>1</p></div>
            <div className="doc__body__cost col-5"><p>{amount.amount? amount.amount.toFixed(2) : ""}</p></div>
            <div className="doc__body__amount col-6"><p>{amount.amount? amount.amount.toFixed(2) : ""}</p></div>
          </div>
          ...
          <div className="doc__body">
            <div className="col-7"><p>Итого:</p></div>
            <div className="doc__body__total col-8"><p>{amount.amount? amount.amount.toFixed(2) : ""}</p></div>
          </div>
          <div className="doc__body">
            <div className="col-7"><p>Без налога (НДС):</p></div>
            <div className="doc__body__totalwotax col-8"></div>
          </div>
          <div className="doc__body">
            <div className="col-7"><p>Всего (с учетом НДС):</p></div>
            <div className="doc__body__totalwtax col-8"><p>{amount.amount? amount.amount.toFixed(2) : ""}</p></div>
          </div>
          <div className="doc__middle">
            <p><strong>Всего услуг на сумму:</strong></p>

            <div className="doc__middle__amount">
              <div className="doc__middle__roubles">
                <p>{textAmount? textAmount.charAt(0).toUpperCase() + textAmount.slice(1) : "" }</p>
          ...
          <div className="doc__bottom">
            <p><strong>Реквизиты сторон:</strong></p>
            <div className="doc__bottom__left">
            <p>
                {userInfo.organization}<br></br>
                {userInfo.legaladdress}<br></br>
                УНП: {userInfo.unp}<br></br>
                {userInfo.bankaccount}<br></br>
                {userInfo.bankname}<br></br>
                {userInfo.bic}<br></br>
              </p>
            </div>
            <div className="doc__bottom__right">
              <p>
                {clientinfo.organization}<br></br>
                {clientinfo.legaladdress}<br></br>
                {clientinfo.unp}<br></br>
                {clientinfo.bankaccount}<br></br>
                {clientinfo.bankname}<br></br>
                {clientinfo.bic}<br></br>
              </p>
            </div>

            <div className="doc__bottom__notice">
              <div className="doc__bottom__notice__content">
                <p>{docType? clientinfo.noticeact : clientinfo.noticeinvoice}</p>
              </div>
            </div>
        ...
        <Buttons onClick={exportPdf} className="main__buttons_btn pdf__btn"><PictureAsPdfIcon fontSize={"large"}/> Экспорт PDF</Buttons>
      </div>
     {/* END PAINTTING TABLE */}
    </>
      );
